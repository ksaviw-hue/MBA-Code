...
public class BasketballLobby
    ...
    private final List<CourtQueue> onesCourts = new ArrayList<>();
    ...
    public BasketballLobby() {
        ...
        this.rankedHalfCourts.add(new Location(Bukkit.getWorlds().getFirst(), 37.5, -61, 271));
        this.rankedHalfCourts.add(new Location(Bukkit.getWorlds().getFirst(), -20.5, -61, 271));
...
//        this.fourVfourDefaultCourts.add(new Location(Bukkit.getWorlds().getFirst(), -283.5, 0.0, 467.5));
        ...
        Bukkit.getLogger().info("[Park Queue] Registered Basket ballLobby as event listener");
    }
...
// For 1v1: Skip jump ball and go straight to game start
    ...
    public void openGameSelectorGUI(Player player) {
        ...
        mode1v1.editMeta(meta -> {
            ...
            meta.lore(List.of(
                ...
                Component.text(" "),
                Component.text("§7Stand on a spot at the 1v1 court!"),
                Component.text("§7Find the court with colored particles"),
    ...
    public void onBlockBreak(org.bukkit.event.block.BlockBreakEvent event) {
        ...
        if (player.getGameMode() == org.bukkit.GameMode.CREATIVE) {
            ...
            for (CourtQueue court : onesCourts) {
    ...
    private void initializePhysicalQueueSpots() {
        ...
        if (rankedCourts.size() >= 4) {
            twosCourts.add(new CourtQueue(rankedCourts.get(0), 2));
            twosCourts.add(new CourtQueue(rankedCourts.get(1), 2));
            
            // Allocate 2 courts for 3v3 (last 2 ranked courts)
            threesCourts.add(new CourtQueue(rankedCourts.get(2), 3));
            threesCourts.add(new CourtQueue(rankedCourts.get(3), 3));
    ...
    private void startParticleTask() {
        particleTask = Bukkit.getScheduler().runTaskTimer(Partix.getInstance(), () -> {
            // Spawn particles for all 1v1 court spots
            for (CourtQueue court : onesCourts) {
    ...
    private String getSpotMessage(CourtSpot spot) {
        String courtType = spot.getCourt().getPlayersPerTeam() == 1 ? "1v1" : (spot.getCourt().getPlayersPerTeam() == 2 ? "2v2" : "3v3");
