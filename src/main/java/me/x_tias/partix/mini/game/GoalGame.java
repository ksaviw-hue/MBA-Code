...
public abstract class GoalGame
    ...
    private final Set<UUID> benchedPlayers = new HashSet<>();
    private final Set<UUID> benchUnderBasket = new HashSet<>();
    ...
    public void startBroadcastFor(final Player p) {
        ...
        final Location base = this.getCenter().clone().add(0.0, 14.0, 0.0);
        base.setYaw(180.0f);
        base.setPitch(35.0f);
    ...
    public void exitBroadcastFor(Player p) {
        ...
        p.setGameMode((p.isOp() || p.hasPermission("rank.admin")) ? GameMode.CREATIVE : GameMode.ADVENTURE);
    ...
    public void reset() {
        ...
        this.points.clear();
        this.threes.clear();
        this.benchedPlayers.clear();
        this.benchUnderBasket.clear();
    ...
    public String getPeriodNameHud() {
        return getNth(this.period);
    }
...
//            System.out.println("AWAY CONTAINS BALL: " + this.awayNet.contains(ballVec));
            ...
            // Only count a goal if the ball is moving downward and has reached the bottom of the net box
            boolean movingDown = this.ball.getVelocity().getY() < 0;
            double bottomThreshold = 0.35;
            if (movingDown && this.homeNet.clone().expand(0.1).contains(ballVec)
                    && ballVec.getY() - this.homeNet.getMinY() < bottomThreshold) {
                this.goal(Team.AWAY);
            } else if (movingDown && this.awayNet.clone().expand(0.1).contains(ballVec)
                    && ballVec.getY() - this.awayNet.getMinY() < bottomThreshold) {
                this.goal(Team.HOME);
            }
        }
    }
    ...
    public void clickItem(Player player, ItemStack itemStack) {
        if (itemStack.getType().equals(Material.OAK_STAIRS)) {
            if (this.isInBench(player)) {
                // Toggle between corner and under-basket
                if (this.benchUnderBasket.contains(player.getUniqueId())) {
                    this.enterBench(player);
                } else {
                    this.enterBenchUnderBasket(player);
                }
            } else {
                this.enterBench(player);
            }
...
//        }
    ...
    public void enterBench(Player player) {
        ...
        // Teleport to back right corner of their team's half (same sideline for both teams)
        benchSpot.setX(arenaBox.getMaxX() + 3.0);
        if (team == Team.HOME) {
            benchSpot.setZ(arenaBox.getMaxZ() - 2.0);
        } else {
            benchSpot.setZ(arenaBox.getMinZ() + 2.0);
        }
        benchSpot.setY(this.getCenter().getY());

        this.benchedPlayers.add(player.getUniqueId());
        this.benchUnderBasket.remove(player.getUniqueId());
    ...
    private void enterBenchUnderBasket(Player player) {
        GoalGame.Team team = getTeamOf(player);
        if (team == null) return;

        // Teleport under the basket their team is defending
        BoundingBox net = (team == Team.HOME) ? this.getHomeNet() : this.getAwayNet();
        Location underBasket = this.getCenter().clone();
        underBasket.setX(net.getCenterX());
        underBasket.setZ(net.getCenterZ());
        underBasket.setY(this.getCenter().getY());

        this.benchUnderBasket.add(player.getUniqueId());
        player.teleport(underBasket);
        Cooldown.setRestricted(player.getUniqueId(), 20);
    }
    ...
    public void leaveBench(Player player) {
        ...
        this.benchedPlayers.remove(player.getUniqueId());
        this.benchUnderBasket.remove(player.getUniqueId());
    ...
    public boolean isInBench(Player player) {
        if (this.benchedPlayers.contains(player.getUniqueId())) return true;
        BoundingBox arenaBox = this.getArenaBox();
